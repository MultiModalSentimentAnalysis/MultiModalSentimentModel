...
class FaceEmotionRecognizer:
    ...
    def compute_probability(self, features):
        return np.dot(features, np.transpose(self.classifier_weights)) + self.classifier_bias
    ...
    def predict_emotions(self, face_img, logits=True):
        ...
        scores = self.compute_probability(features)[0]
        if self.is_mtl:
            x = scores[:-2]
        else:
            x = scores
    ...
    def extract_multi_features(self, input_faces):
        faces = [self.test_transforms(Image.fromarray(face_img)) for face_img in input_faces]
        features = self.model(torch.stack(faces, dim=0).to(self.device))
        features = features.data.cpu().numpy()
        return features

    def predict_multi_emotions(self, face_img_list, logits=True, return_features=True):
        features = self.extract_multi_features(face_img_list)
        scores = self.compute_probability(features)
        if self.is_mtl:
